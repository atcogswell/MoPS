...
route<-leaflet(data4) %>%
  ...
  addCircles(lng=tpts$lon_dd,lat=tpts$lat_dd, weight = 5, radius=10, color="red", stroke = TRUE,opacity=0.5,group="Transit Locations",
             fillOpacity = 1,popup=paste ("ID:",tpts$ID,"|", "Station:", tpts$type,"|","Lon DD:", round(tpts$lon_dd,3), "|","Lat DD:",round(tpts$lat_dd,3),"|","Lat DMS:",tpts$lat_dms,"|","Lon DMS:",tpts$lon_dms,"|","Arrival:",substrLeft(tpts$arrival,16),"|","Departure:",substrLeft(tpts$departure,16), "Next Stn:",round(tpts$dist_nm,1),"nm","&",round(tpts$trans_hr,1),"hr(s)",sep=" "))%>%
