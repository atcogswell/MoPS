...
s<-ISOdate(2017, 11, 24, 10) #start date and time for mission (Year, month, day, 24hr time)

#### 5. Choose your input file ----
#file=file.choose()
file<-"AZMPFall2017_config19.csv"
#file="HUD2017000_config5.csv"
data<-read.csv(file, stringsAsFactors=F, fileEncoding="Latin1")
file2<-basename(file)
...
data$lat_dm<-paste(latDeg," ",latmin2, " N", sep="")
...
data$lon_dm<-paste(lonDeg," ", lonmin2, " W",sep="")
...
if (exists('depth')) depth <- depth else depth <- readAsciiGrid(rwd, proj4string=CRS("+proj=longlat +datum=WGS84"))#assigns ASCII grid from rwd to variable name
data1<-data[,1:2]
...
route<-leaflet(data4) %>%
  ...
  addCircles(lng=opts$lon_dd, lat=opts$lat_dd, weight = 5, radius=10, color="yellow",stroke = TRUE, opacity=.5,group="Operations Locations",
             fillOpacity = 1, popup=paste ("ID:",opts$ID,"|", "Station:", opts$station,"|","Lon:", round(opts$lon_dd,3), "|","Lat:",round(opts$lat_dd,3), "|","Depth:",round(opts$depth_m,1),"m","|", "Arrival:",substrLeft(opts$arrival,16),"|","Departure:",substrLeft(opts$departure,16), "|","Op Time:",(opts$optime+opts$xoptime),"hr(s)","|","Operation(s):",opts$operation, "|","Next Stn:",round(opts$dist_nm,1),"nm","&",round(opts$trans_hr,1),"hr(s)",sep=" "))%>% 
  addCircles(lng=moorings$lon_dd, lat=moorings$lat_dd, weight = 10, radius=10, color="green",stroke = TRUE, opacity=1,group="Mooring Locations",
             fillOpacity = 1, popup=paste ("ID:",moorings$ID,"|", "Station:", moorings$station,"|","Lon:", round(moorings$lon_dd,3), "|","Lat:",round(moorings$lat_dd,3), "|","Depth:",round(moorings$depth_m,1),"m","|", "Arrival:",substrLeft(moorings$arrival,16),"|","Departure:",substrLeft(moorings$departure,16), "|","Op Time:",(moorings$optime+moorings$xoptime),"hr(s)","|","Operation(s):",moorings$operation, "|","Next Stn:",round(moorings$dist_nm,1),"nm","&",round(moorings$trans_hr,1),"hr(s)",sep=" "))%>%
  ...
  addLayersControl(
  overlayGroups = c("Operations Locations","Transit Locations","Mooring Locations","Route","Labels"),
