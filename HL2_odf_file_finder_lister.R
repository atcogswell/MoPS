...
odf_file_lister <- function(working_directory, year, site_code){
  ...
  if(!use_src){
    ...
    only_666 <- grepl(pattern = paste(site_code, "_", sep = ""), x = odf_file_list_i)
    only_DN <- grepl(pattern = "_DN", x = odf_file_list_i)
    if(year > 1999){
      only_bcd <- grepl(pattern = "BCD", x = odf_file_list_i)
      odf_file_list_i <- odf_file_list_i[only_666 & only_bcd & only_DN]
    } else if (year == 1999){
      only_99666 <- grepl(pattern = paste("99", site_code, sep = ""), x = odf_file_list_i)
      odf_file_list_i <- odf_file_list_i[only_99666]
    }
  } else if(use_src){
    odf_file_list_i <- list.files(pattern="*^.*D.*.ODF$")    
  }
